...
def get_embedder():
    ...
    if device == "cpu":
        ...
        # attn_implementation="eager" i bezpośrednio wywołują xformers, które
        # wymaga CUDA. Wyłączamy xformers ręcznie po załadowaniu modelu.
        _disable_xformers_cpu(model)
    else:
        try:
            model = model.half()
        except Exception:
